 ...
 * <p>
 * 【要求】
 * 如果数组arr的长度为N，排序之后自然可以得到最小的k个数，此时时间复杂度与排
 * 序的时间复杂度相同，均为O(NlogN)。本题要求读者实现时间复杂度为O(Nlogk)和O(N)
 * 的方法。
 */
public class _04GetMinKNums {
    ...
    public int[] getMinKNumsByBFPRT(int[] arr, int k) {
        ...
        int minKth = getMinKthByBcFPRT(arr, k);
        int[] res = new int[k];
        int index = 0;
    ...
    private int getMinKthByBcFPRT(int[] arr, int k) {
